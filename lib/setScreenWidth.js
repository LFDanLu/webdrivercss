...
module.exports = function(done) {
    ...
    async.waterfall([
        ...
        function(cb) {
            ...
            if(!that.screenHeight || that.screenHeight.length ===0) {
                that.newScreenSize.height = parseInt(that.self.defaultScreenDimension.height-100, 10);
            }
            else{
                that.newScreenSize.height = parseInt(that.self.screenHeight.shift(), 10);
            }
            ...
            that.instance.call(function() {
                 ...
                 * value in filename
                 */
                that.baselinePath   = that.baselinePath.replace(/\.(baseline|regression|diff)\.png/,'.' + that.newScreenSize.width + "x" + that.newScreenSize.height + 'px.$1.png');
                that.regressionPath = that.regressionPath.replace(/\.(baseline|regression|diff)\.png/,'.' + that.newScreenSize.width + "x" + that.newScreenSize.height + 'px.$1.png');
                that.diffPath       = that.diffPath.replace(/\.(baseline|regression|diff)\.png/,  '.' + that.newScreenSize.width + "x" + that.newScreenSize.height + 'px.$1.png');
                that.screenshot     = that.screenshot.replace(/\.png/, '.' + that.newScreenSize.width + "x" + that.newScreenSize.height + 'px.png');
